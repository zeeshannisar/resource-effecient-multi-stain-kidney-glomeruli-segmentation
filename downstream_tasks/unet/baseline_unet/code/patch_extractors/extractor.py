...
class ExtractMethod(object):
    ...
    def extract_patches(self, patients, className, classLabel, nbPatches, debug=False):
        ...
        # Get the gt path for each image
        ...
        imageLabelPath = self.file_path.generate_groundtruthlabelspath(listNames[0], self.lod)
        ...
        nbPatchPerImage = self.compute_nb_patches_per_image(nbPatches, listgtPaths, classLabel)

        if numpy.any(nbPatchPerImage):
            self.create_folders(className, debug=debug)

        imageLabelPath = self.file_path.generate_groundtruthlabelspath(listNames[0], self.lod)
        if os.path.isfile(os.path.join(self.output_folder, 'class_labels.json')):
            # check that gt labels are the same as existing
            with open(imageLabelPath, 'r') as fp:
                imageLabels = json.load(fp)
            patchLabelPath = os.path.join(self.output_folder, 'class_labels.json')
            with open(patchLabelPath, 'r') as fp:
                patchLabels = json.load(fp)
            if not imageLabels == patchLabels:
                raise ValueError("Image labels differ from existing patch labels")
        else:
            # copy gt label file
            copyfile(imageLabelPath, os.path.join(self.output_folder, 'class_labels.json'))

        gtLabels = None
        for imageName, imagepath, gtpath, nb in zip(listNames, listImagePaths, listgtPaths, nbPatchPerImage):
            print("Image: %s" % imageName)

            gt = image_utils.read_image(gtpath).astype(numpy.uint8)

            labelsPath = self.file_path.generate_groundtruthlabelspath(imageName, self.lod)
            with open(labelsPath, 'r') as fp:
                newgtLabels = json.load(fp)
            if not gtLabels:
                gtLabels = newgtLabels

            if not newgtLabels == gtLabels:
                raise ValueError("%s's ground truths differ from previously analysed images, cannot mix class definitions" % (imageName))
            ...
            patch_coords = self.get_patch_coords(gt, classLabel, nb)
            ...
            self.create_patches(imagepath, gt, patch_coords, className, imageName, True, debug)
    ...
    def extract_tissue_patches(self, patients, className, nbPatches, debug=False):
        ...
        listImagePaths = self.file_path.get_images_with_list_patients(patients=patients)
        listNames = [ip[1] for ip in listImagePaths]
        listImagePaths = [ip[0] for ip in listImagePaths]

        maskPaths = []
        for imageName, imagepath in zip(listNames, listImagePaths):
            maskPaths.append(self.file_path.get_mask_for_image(imageName, 'tissue', self.lod)[0])

        # compute the number of patches requested
        nbPatchPerImage = self.compute_nb_patches_per_image(nbPatches, maskPaths, 1)

        if numpy.any(nbPatchPerImage):
            self.create_folders('', writegtpatches=False, debug=debug)
        ...
        for imageName, imagepath, maskpath, nb in zip(listNames, listImagePaths, maskPaths, nbPatchPerImage):
            ...
            self.create_patches(imagepath, [], patch_coords, '', imageName, debug)
    ...
    def create_patches(self, imagepath, gt, patch_coords, className, imageName, writegtpatches=True, debug=False):
        ...
        for i, coords in enumerate(patch_coords):
            ...
            if writegtpatches:
                patchMask = gt[coords[1] - patch_shift: coords[1] + patch_shift, coords[0] - patch_shift: coords[0] + patch_shift]
                outputMaskPath = os.path.join(self.output_folder, "gts", className, imageName + "_" + className + "_patch_" + str(i) + ".png")
                image_utils.save_image(patchMask, outputMaskPath)
    ...
    def create_folders(self, className, writegtpatches=True, debug=False):
        """
        ...
        if writegtpatches and not os.path.exists(os.path.join(outputDir, 'gts', className)):
