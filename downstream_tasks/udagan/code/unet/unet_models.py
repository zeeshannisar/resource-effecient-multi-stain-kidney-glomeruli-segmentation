...
def getvalidinputsize(inp_shape, depth=5, k_size=3, data_format='channels_last'):
    ...
    for d in spatial_dims:
        ...
        if new_inp_shape_d > inp_shape[d]:
            raise ValueError('Minimum possible image size is larger than the original image size, increase patch resolution or patch size.')
...
def getoutputsize(inp_shape, depth=5, k_size=3, padding='valid', data_format='channels_last'):
    ...
    def calculate_bridge_end_size(dim_size):
        ...
        if not dim_size.is_integer():
            raise ValueError('This input shape produces a non-integer feature map shape, use getvalidinputsize to calculate a valid input shape.')
        dim_size -= (k_size-1)*2
    ...
    def calculate_output_size(dim_size):
        ...
        if not dim_size.is_integer():
            raise ValueError('This input shape produces non-integer feature map size, use getvalidinputsize to calculate a valid input shape.')
...
def baseline_unet_model(inp_shape, nb_classes, depth=5, filter_factor_offset=0, initialiser='glorot_uniform',
    ...
    """
    print("\nInitializing the Baseline UDAGAN Network...")
    ...
    model = Model(data, out, name='Baseline_UDAGAN_model')
...
def SimCLR(simclr_model_path, simclr_model_trainable, inp_shape, depth=5, filter_factor_offset=0,
    ...
    if simclr_model_trainable:
        print("\nInitializing the SimCLR based Finetune UDAGAN Network...")
    else:
        print("\nInitializing the SimCLR based Fixed-Features UDAGAN Network...")
...
def BYOL(byol_model_path, byol_model_trainable, inp_shape, depth=5, filter_factor_offset=0, initialiser='glorot_uniform',
    ...
    if byol_model_trainable:
        print("\nInitializing the BYOL based Finetune UDAGAN Network...")
    else:
        print("\nInitializing the BYOL based Fixed-Features UDAGAN Network...")
    name = 'byol'
    if padding == 'valid':
        inp_shape = getvalidinputsize(inp_shape, depth, k_size)
...
def unet_decoder_above_SSL(pretrained_models, inp_shape, nb_classes, depth=5, filter_factor_offset=0,
    ...
    def unet_decoder(pretrained_models):
        ...
        if 'byol_model' in pretrained_models:
        ...
        if 'simclr_model' in  pretrained_models:
            return Model(pretrained_models['simclr_model'].input, out, name='SimCLR_based_UDAGAN_model'), inp_shape, otp_shape
        elif 'byol_model' in pretrained_models:
            return Model(pretrained_models['byol_model'].input, out, name='BYOL_based_UDAGAN_model'), inp_shape, otp_shape
        else:
