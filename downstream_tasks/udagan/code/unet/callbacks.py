...
class CheckpointTrainingPatches(Callback):
    ...
    def __init__(self, source_dir, patches_dir, inp_shape, otp_shape, colour_mode, mean=None, stddev=None):
        ...
        self.mean = mean
        self.stddev = stddev
        self.source_images_test, self.source_masks_test, self.source_class_number, self.source_filenames = self.__getdataset__(
            source_dir, colour_mode, True, 8)
        self.output_path = patches_dir

    def __getdataset__(self, input_patch_path, colour_mode, standardise_patches=False, number_of_patches_per_class=5):
    ...
    def __predictions__(self, images_test, filenames, class_number, model, output_path, epoch):
        ...
        for pred, filename in zip(y_pred, filenames):
            for c in range(class_number):
                image_utils.save_image((pred[..., c] * 255).astype(numpy.uint8), os.path.join(img_output_path,
                                                                                              os.path.splitext(
                                                                                                  os.path.basename(
                                                                                                      filename))[
                                                                                                  0] + '_' + str(
    ...
    def on_epoch_end(self, epoch, logs):
        self.__predictions__(self.source_images_test, self.source_filenames, self.source_class_number, self.model,
                             self.output_path, epoch)
...
class DisplayLearningRateValue(Callback):
    def on_epoch_end(self, epoch, logs=None):
        print(f"current_LR: {format(self.model.optimizer.lr.numpy().astype('float32'), '.8f')} at Epoch: {epoch+1}\n")
