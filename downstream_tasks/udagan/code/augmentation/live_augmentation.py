...
from augmentation import elastic_transform, affine_transform, stain_transform, blur_transform, noise_transform, \
    enhance_transform, channel_transform, stain_transfer_transform
...
from utils import data_utils, image_utils, config_utils_stargan
...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        self.stain_transfer_funs = {}
        self.stain_transfer_choice = []
        if 'stain_transfer' in self.methods:
            list_of_epochs = self.augmentationparameters['model_suffixes']
            stains = self.augmentationparameters['colour_transfer_targets']

            base_stain = os.path.basename(self.augmentationparameters['gan_model_path'])
            colour_transfer_targets = []
            if len(list_of_epochs) > 0:
                for stain in stains:
                    if stain != base_stain:
                        for epoch in list_of_epochs:
                            colour_transfer_targets.append(stain + '_' + epoch)
            else:
                colour_transfer_targets = stains
                if base_stain in colour_transfer_targets:
                    colour_transfer_targets.remove(base_stain)

            if self.augmentationparameters['transfer_choice'] == 'on_fly':
                print(f"style_transfer_choice: {self.augmentationparameters['transfer_choice']} --- "
                      f"style_transfer_targets: {colour_transfer_targets}")
                if self.augmentationparameters['transfer_model'] == 'cyclegan':
                    for target_stain in colour_transfer_targets:
                        transform_model = CycleGAN_models.CycleGAN(stain=target_stain)
                        transform_model.combined.load_weights(os.path.join(self.augmentationparameters['gan_model_path'],
                                                                           target_stain, 'combined.h5'))
                        self.stain_transfer_funs[target_stain] = transform_model.g_AB
                        self.stain_transfer_funs[target_stain].trainable = False
                        self.stain_transfer_funs[target_stain].make_predict_function()
            else:
                print(f"style_transfer_choice: {self.augmentationparameters['transfer_choice']} --- "
                      f"style_transfer_targets: {colour_transfer_targets}")
                for target_stain in colour_transfer_targets:
                    self.stain_transfer_choice.append(f"02_to_{target_stain}")
    ...
    def random_transform(self, x, y, j=None, images_dir=None, filename=None, staincode=None, block=[]):
        ...
        # Translate patches without any modification first
        if 'stain_transfer' not in block:
            # if not staincode in self.augmentationparameters['colour_transfer_targets'] or \
            #     (staincode in self.augmentationparameters['colour_transfer_targets'] and random.random() > 1 / 5):
            if not staincode in self.augmentationparameters['colour_transfer_targets'] and random.random() > 1 / 5:
                alpha_range = None
                if 'stain' in self.methods:
                ...
                if self.augmentationparameters['transfer_choice'] == 'on_fly':
                    x, y = stain_transfer_transform.transform(x, y, self.stain_transfer_funs, 'stain' in self.methods,
                                                          alpha_range=alpha_range, beta_range=beta_range)
                else:
                    x, y = stain_transfer_transform.already_saved(x, y, self.stain_transfer_choice,
                                                                  self.augmentationparameters['saved_translations_path'],
                                                                  images_dir, filename, 'stain' in self.methods,
                                                                  alpha_range=alpha_range, beta_range=beta_range)
        ...
        for i in ord:
            ...
            if 'affine' not in block and self.methods[i] == 'affine' and random.random() < 0.5:
                x, y = affine_transform.transform(x, y,
                                                  ...
                                                  width_shift_range=self.augmentationparameters[
                                                      'affine_width_shift_range'],
                                                  height_shift_range=self.augmentationparameters[
            ...
            if 'stain' not in block and self.methods[
                i] == 'stain' and 'stain_transfer' not in self.methods and random.random() < 0.5:
            ...
            if 'enhance' not in block and self.methods[i] == 'enhance' and random.random() < 0.5:
                x, y = enhance_transform.transform(x.astype('uint8'), y,
                                                   bright_factor_range=self.augmentationparameters[
                                                       'bright_factor_range'],
                                                   contrast_factor_range=self.augmentationparameters[
                                                       'contrast_factor_range'],
                                                   colour_factor_range=self.augmentationparameters[
                                                       'colour_factor_range'])
    ...
    def fit(self, x, y,
        ...
        if 'stain' in self.methods and 'stain_transfer' in self.methods:
            warnings.warn(
                'Using ''stain'' augmentation with the stain_transfer training strategy is not currently advised as the '
                'resulting normalisation statistics may not match realistic values')
...
class DirectoryIterator(Iterator):
    ...
    #     return 0.7377701673822199, 0.22505307146854556, numpy.zeros(self.nb_classes, dtype=numpy.uint)
    ...
    def get_fit_stats(self, standardise_samples):
        ...
        def get_mean_std(filename, patchclass):
            ...
            y = load_img(os.path.join(self.directory, 'gts', filename),
                         grayscale=True, data_format=self.data_format).astype(
                numpy.uint)
            ...
            if self.filepath:
                ...
                x, y = self.image_data_generator.random_transform(x, y, images_dir=self.directory, filename=filename,
                                                                  staincode=staincode, block=blockedaugmentations)
            else:
                x, y = self.image_data_generator.random_transform(x, y, images_dir=self.directory, filename=filename,
                                                                  block=blockedaugmentations)
        ...
        for batch_mean, batch_var, s, sample_class_weights in samples:
            ...
            for i, c in enumerate(sample_class_weights):
                class_weights[i] += c
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            x = load_img(os.path.join(self.directory, 'images', fname),
                         grayscale=False, data_format=self.data_format)
            y = load_img(os.path.join(self.directory, 'gts', fname),
                         grayscale=True, data_format=self.data_format)
            ...
            if self.filepath:
                ...
                x, y = self.image_data_generator.random_transform(x, y, images_dir=self.directory, filename=fname,
                                                                  staincode=staincode, block=blockedaugmentations)
            else:
                x, y = self.image_data_generator.random_transform(x, y, images_dir=self.directory,
                                                                  filename=fname, block=blockedaugmentations)
