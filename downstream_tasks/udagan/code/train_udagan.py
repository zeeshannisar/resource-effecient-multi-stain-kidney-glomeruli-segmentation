...
def trainudagan(config, number_of_classes, class_weights, label=None):
    """
    ...
    if not check_gt_validity(os.path.join(config['detector.traininputpath'], 'train', 'class_labels.json'),
        ...
        raise ValueError("Training patch class definitions differ from those defined in the UNet configuration file")
   # check valid class labels
    ...
    if config['detector.LR_weightdecay'] is not None:
        optimiser = tfa.optimizers.AdamW(learning_rate=config['detector.learn_rate'], beta_1=0.9, beta_2=0.999,
                                         epsilon=1e-08, weight_decay=config['detector.LR_weightdecay'],
                                         name='AdamW')
    else:
        optimiser = tf.keras.optimizers.Adam(learning_rate=config['detector.learn_rate'], beta_1=0.9, beta_2=0.999,
                                             epsilon=1e-08, decay=0.0, name='Adam')
    ...
    if config['detector.transferlearning']:
        ...
        UNet, inp_shape, otp_shape = unet_decoder_above_SSL(pretrained_models,
                                                            ...
                                                            filter_factor_offset=config[
                                                                'detector.filter_factor_offset'],
    ...
    if config['normalisation.normalise_patches']:
        ...
        # save_to_dir = config['augmentation.patchpath'])
    ...
    if config['detector.validationinputpath'] is not None: 
        UNet.fit(train_flow, epochs=config['detector.epochs'], shuffle=True, callbacks=callbacklist,
                 verbose=2, validation_data=valid_flow)#, use_multiprocessing=True, workers=8, max_queue_size=650)
    else:
        UNet.fit(train_flow, epochs=config['detector.epochs'], shuffle=True, callbacks=callbacklist,
...
def pretrained_simclr_model_path(conf):
    return os.path.join(conf['general.homepath'], 'code/improve_kidney_glomeruli_segmentation',
                        'pre_training/ssl_pretrained_models/simclr/simclr_unet_encoder.h5')

def pretrained_byol_model_path(conf):
    return os.path.join(conf['general.homepath'], 'code/improve_kidney_glomeruli_segmentation',
                        'pre_training/ssl_pretrained_models/byol/byol_unet_encoder.h5')
...
def derived_parameters(conf, arguments):
    ...
    if conf['detector.validation_data'] is None:
        conf['detector.modelpath'] = os.path.join(conf['detector.modelpath'], 'downstream_tasks_evaluation', conf['detector.patchstrategy'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--label', type=str, help='saved model name')
    ...
    trainudagan(config, number_of_classes, class_weights, label=args.label)
