...
from deepmodels.functional_API import ResNet50, UNet, DeepResidualUNet
...
class Training:
    ...
    def train(self):
        ...
        with strategy.scope():
            ...
            elif self.config["training.DeepModel"].lower() == "deepresidualunet":
                model = DeepResidualUNet(config=self.config).model()
        ...
        if StartEpoch >= self.config["training.NumEpochs"]:
            ...
            for epoch in range(StartEpoch, self.config["training.NumEpochs"]):
                ...
                for step, x in enumerate(dataset_distributed):
                    ...
                    # if step % (self.config["training.SaveInterval"] * self.config["training.BatchSize"]) == 0:
                    #     print(f"Epoch: {epoch}/{self.config['training.NumEpochs']} - Batch: {step}")
                ...
                if epoch % self.config["training.SaveInterval"] == 0:
                    tensorboard.save_to_dir(dataset=dataset, epoch=epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--configuration_file', type=str, default="configuration_files/simclr.cfg")
    parser.add_argument('-m', '--DeepModel', type=str, default="unet", help='deep learning network')
