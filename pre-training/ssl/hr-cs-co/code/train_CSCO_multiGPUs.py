...
def TF_ExtractCentralRegion(images, shape=(324, 324)):
    margin_crop_ratio = shape[0]/images.shape[1]
    return tf.map_fn(lambda image: tf.image.central_crop(image, margin_crop_ratio), images)
...
def SaveModelArchitecture(encoder, decoder, combined_encoder, target_combined_encoder, combined_decoder, post_process,
                          target_post_process, projector, target_projector, predictor, input_shape, savePath):
    ...
    h_encoder_output, o_encoder_output, h_skip0, h_skip1, h_skip2, h_skip3, \
    o_skip0, o_skip1, o_skip2, o_skip3 = target_combined_encoder(tf.ones(shape=(1, *input_shape)),
                                                                 tf.ones(shape=(1, *input_shape)), training=True)
    WriteModelArchitecture(model=target_combined_encoder.build_graph(h_input_shape=input_shape, o_input_shape=input_shape),
                           path=savePath, name="Target_HO_UnetEncoder")
    ...
    post_process_output = target_post_process(h_encoder_output, o_encoder_output)
    WriteModelArchitecture(model=target_post_process.build_graph(h_encoder_output.shape[1:], o_encoder_output.shape[1:]),
                           path=savePath, name="Target_PostProcess_HOEncoder")
    ...
    projector_output = target_projector(post_process_output)
    WriteModelArchitecture(model=target_projector.build_graph(post_process_output.shape[1:]), path=savePath, name="Target_Projector")
...
class Training:
    def __init__(self, tmp_config, pretrained_model_path=None):
        ...
        with self.strategy.scope():
            if self.config["training.DeepModel"].lower() == "unet":
                ...
                self.combined_encoder, self.combined_decoder, self.target_combined_encoder = HO_UnetEncoder(), HO_UnetDecoder(), HO_UnetEncoder()
            ...
            # Target Networks
            self.target_post_process = PostProcessHOEncoder()
            self.target_projector = ProjectionHead()
            self.optimizer = SelectOptimizer(self.config)

            print("\n")
            SaveModelArchitecture(self.encoder, self.decoder, self.combined_encoder, self.target_combined_encoder,
                                  self.combined_decoder, self.post_process, self.target_post_process,
                                  self.projector, self.target_projector, self.predictor, self.input_shape,
                                  savePath=f"{self.config['output.OutputDir']}/network_summary")

            if pretrained_model_path is not None:
                print("Loading Pretrained Encoder Model Weights...")
                ...
                self.target_combined_encoder.load_weights(os.path.join(pretrained_model_path, f"HO_encoder_model.best.hdf5"))
                self.target_combined_encoder.trainable = True
                print("Loading Pretrained Decoder Model Weights...")
    ...
    def train(self):
        ...
        train_dataset_distributed = self.strategy.experimental_distribute_dataset(self.ds_train)
        valid_dataset_distributed = self.strategy.experimental_distribute_dataset(self.ds_valid)

        with self.strategy.scope():
            def DecoderLoss(predictions, labels, loss_type="mse_loss"):
                if loss_type == "mae_loss":
                    outputs = tf.abs(tf.subtract(predictions, labels))
                    return tf.reduce_mean(outputs, axis=list(range(1, len(labels.shape))))
                if loss_type == "mse_loss":
                    """ return the per sample mean squared error """
                    outputs = tf.square(tf.subtract(predictions, labels))
                    return tf.reduce_mean(outputs, axis=list(range(1, len(labels.shape))))

            def ByolLoss(opred, tproj):
                opred = tf.math.l2_normalize(opred, axis=1)
                tproj = tf.math.l2_normalize(tproj, axis=1)

                similarities = 2 - 2 * (tf.reduce_sum(tf.multiply(opred, tproj), axis=1))
                return similarities

            def CsCoLoss(online_pred_T1, online_pred_T2, target_proj_T1, target_proj_T2, H2O_decoder_T1, O2H_decoder_T1,
                         labels_O_T1, labels_H_T1, decoder_loss_type, decoder_loss_w, byol_loss_w):
                per_sample_decoder_loss = tf.reduce_sum(DecoderLoss(H2O_decoder_T1, labels_O_T1, decoder_loss_type)) / self.config["training.BatchSize"]
                per_sample_decoder_loss += tf.reduce_sum(DecoderLoss(O2H_decoder_T1, labels_H_T1, decoder_loss_type)) / self.config["training.BatchSize"]

                per_sample_byol_loss = tf.reduce_sum(ByolLoss(opred=online_pred_T1, tproj=target_proj_T2)) / self.config["training.BatchSize"]
                per_sample_byol_loss += tf.reduce_sum(ByolLoss(opred=online_pred_T2, tproj=target_proj_T1)) / self.config["training.BatchSize"]

                per_sample_csco_loss = (byol_loss_w * per_sample_byol_loss) + (decoder_loss_w * per_sample_decoder_loss)
                return per_sample_csco_loss, per_sample_byol_loss, per_sample_decoder_loss

        def train_on_batch(images_H_T1, images_H_T2, images_O_T1, images_O_T2, labels_O_T1, labels_H_T1):
            labels_H_T1 = tf.map_fn(lambda image: tf.image.central_crop(image, 324 / 508), labels_H_T1)
            labels_O_T1 = tf.map_fn(lambda image: tf.image.central_crop(image, 324 / 508), labels_O_T1)

            if self.config["preprocessing.NormalizeMeanStd"]:
                images_H_T1 = tf.map_fn(lambda image: normalise(image, self.config["data.mean"], self.config["data.mean"]), images_H_T1)
                images_H_T2 = tf.map_fn(lambda image: normalise(image, self.config["data.mean"], self.config["data.mean"]), images_H_T2)
                images_O_T1 = tf.map_fn(lambda image: normalise(image, self.config["data.mean"], self.config["data.mean"]), images_O_T1)
                images_O_T2 = tf.map_fn(lambda image: normalise(image, self.config["data.mean"], self.config["data.mean"]), images_O_T2)
        ...
        def validation_on_batch(images_H_T1, images_H_T2, images_O_T1, images_O_T2, labels_O_T1, labels_H_T1):
            labels_H_T1 = tf.map_fn(lambda image: tf.image.central_crop(image, 324 / 508), labels_H_T1)
            labels_O_T1 = tf.map_fn(lambda image: tf.image.central_crop(image, 324 / 508), labels_O_T1)
        ...
        @tf.function
        def distributed_train_on_batch(images_H_T1, images_H_T2, images_O_T1, images_O_T2, labels_O_T1, labels_H_T1):
            h2o_output, o2h_output, per_GPU_csco_loss, per_GPU_byol_loss, \
                per_GPU_decoder_loss = self.strategy.run(train_on_batch, args=(images_H_T1, images_H_T2, images_O_T1,
                                                                               images_O_T2, labels_O_T1, labels_H_T1))
            all_GPUs_csco_loss = self.strategy.reduce("SUM", per_GPU_csco_loss, axis=None)
            all_GPUs_byol_loss = self.strategy.reduce("SUM", per_GPU_byol_loss, axis=None)
            all_GPUs_decoder_loss = self.strategy.reduce("SUM", per_GPU_decoder_loss, axis=None)

            return h2o_output, o2h_output, all_GPUs_csco_loss, all_GPUs_byol_loss, all_GPUs_decoder_loss

        @tf.function
        def distributed_validation_on_batch(images_H_T1, images_H_T2, images_O_T1, images_O_T2, labels_O_T1, labels_H_T1):
            h2o_output, o2h_output, per_GPU_csco_loss, per_GPU_byol_loss, \
                per_GPU_decoder_loss = self.strategy.run(validation_on_batch, args=(images_H_T1, images_H_T2, images_O_T1,
                                                                                    images_O_T2, labels_O_T1, labels_H_T1))
            all_GPUs_csco_loss = self.strategy.reduce("SUM", per_GPU_csco_loss, axis=None)
            all_GPUs_byol_loss = self.strategy.reduce("SUM", per_GPU_byol_loss, axis=None)
            all_GPUs_decoder_loss = self.strategy.reduce("SUM", per_GPU_decoder_loss, axis=None)
        ...
        if StartEpoch >= self.config["training.NumEpochs"]:
            ...
            for epoch in range(StartEpoch, self.config["training.NumEpochs"]):
                ...
                for step, inputs in enumerate(train_dataset_distributed):
                    ...
                    h2o_output, o2h_output, csco_loss, byol_loss, decoder_loss = distributed_train_on_batch(image_H_T1,
                                                                                                            image_H_T2,
                                                                                                            image_O_T1,
                                                                                                            image_O_T2,
                                                                                                            label_O_T1,
                    ...
                    if step % 500 == 0:
                        if self.strategy.num_replicas_in_sync > 1:
                            image_H_T1, image_O_T1, label_H_T1, label_O_T1 = image_H_T1.values, image_O_T1.values, label_H_T1.values, label_O_T1.values
                            h2o_output, o2h_output = h2o_output.values, o2h_output.values

                        name = "epoch_" + str(epoch + 1) + "_step_" + str(step)
                        self.tensorboard.save_to_dir(image_H_T1[0], image_O_T1[0],
                                                     label_H_T1[0], label_O_T1[0], h2o_output[0], o2h_output[0], name)

                for step, inputs in enumerate(valid_dataset_distributed):
                    _, _, image_H_T1, label_O_T1, image_O_T1, label_H_T1, \
                    image_H_T2, label_O_T2, image_O_T2, label_H_T2 = inputs

                    h2o_output, o2h_output, csco_loss, byol_loss, decoder_loss = distributed_validation_on_batch(image_H_T1,
                                                                                                                 image_H_T2,
                                                                                                                 image_O_T1,
                                                                                                                 image_O_T2,
                                                                                                                 label_O_T1,
                    ...
                    if step == 0:
                        if epoch % self.config["training.SaveInterval"] == 0:
                            ...
                            self.tensorboard.save_to_dir(image_H_T1[0], image_O_T1[0],
                                                         label_H_T1[0], label_O_T1[0], h2o_output[0], o2h_output[0], name)
                            self.tensorboard.save_to_tensorboard(image_H_T1[0], image_O_T1[0], label_H_T1[0],
                                                                 label_O_T1[0], h2o_output[0], o2h_output[0], epoch + 1)
